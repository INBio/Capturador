...
public class ReportsFacadeImpl implements ReportsFacadeRemote {
    ...
    public boolean reloadDarwinCoreTable(){
        return darwinCore14EAOImpl.reloadDarwinCoreTable();
    }
    ...
    public boolean reloadPlinianCoreTable(){
        return plinianCoreFlatEAOImpl.reloadPlinianCoreTable();
    }
    ...
    public boolean makeDcwSnapshotNative(LinkedList<QueryNode>
            qnlist,LinkedList<String> elist){
        try{
            //Delete all snapshot entries
            dwcSnapshotEAOImpl.truncateDwcSnapshot(); 
        }
        catch(Exception e){return false;}

        //Generate the query string
        String q = makeQueryStringPostgres(qnlist,elist);
    ...
    public boolean makePlicSnapshotNative(LinkedList<QueryNode>
        ...
        try{
            //Delete all snapshot entries
            plicSnapshotEAOImpl.truncatePlicSnapshot();
        }
        catch(Exception e){return false;}

        //Generate the query string
        String q = makeQueryStringPostgresPlic(qnlist,elist);
    ...
    public boolean export(File f) {
        if (f == null) {
            ...
            return this.dwcSnapshotEAOJDBCImpl.writeDwcSnapshotToFile(f);
        }
    }
     ...
     */
    //TODO
    public boolean exportToPlic(File f) {
        if (f == null) {
            return false;
        } else {
            return this.plicSnapshotEAOJDBCImpl.writePlicSnapshotToFile(f);
        }
    }
	...
	private String makeQueryStringPostgresPlic(LinkedList<QueryNode> llqn,
            LinkedList<String> lls) {
        //Construyecto el insert
        String jpqlQuery = "insert into ara.plic_snapshot (";
        int llsSize = lls.size();
        for(int i = 0;i<llsSize;i++){
            String element = lls.get(i).toLowerCase();
            if(i==(llsSize-1)) //Si es el ultimo, no debe llevar coma (,)
        ...
        //Construyecto el from
        jpqlQuery+="from ara.plinian_core_flat plic ";
        if (llqn.isEmpty()){ //si no hay criterio de búsqueda
