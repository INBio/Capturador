...
public class NewTaxonomy extends AbstractPageBean {
    ...
    */
    ...
    private TextArea txaCountryDescription = new TextArea();
       ...
       if(tsb.getTaxonomicalRangeSelected() == null)
       {
        ...
        if(tsb.getTaxonTabSelected().equals("tabTaxonCountry"))
        {
            ...
            if(tsb.getCountries().size() == 0)
            {
                ...
                Set<Option> countryOptions = new HashSet<Option>();

                for(CountryDTO countryDto: countryList)
                {
                    countryOptions.add(new Option(countryDto.getCountryId(), countryDto.getValue()));
                }
                tsb.setCountries(countryOptions);
            }
    ...
    public String btnSaveTaxon_action() {
                ...
                (TACSB.getIdSelected() == null && TACSB.getText().length() ==0))
        {
            if(this.getHiddenTaxonNodeId().getValue() != null){
               ...
               /*CREATE SYNONYMS*/
               if(TSB.getSynonymList() != null && TSB.getSynonymList().size() > 0)
               {
                   List<Long> tmpSynonyms = new ArrayList<Long>();
                   for(SynonymDTO synonym: TSB.getSynonymList())
                   {
                       tmpSynonyms.add(synonym.getTaxonKey());
                   }
                   //TSB.saveSynonyms(TSB.getCurrentTaxon().getTaxonKey(), TSB.getSynonymList());
                   TSB.saveSynonyms(TSB.getCurrentTaxon().getTaxonKey(), tmpSynonyms);
               }
               ...
               /* CREATE NEW TAXON-COUNTRY RELATIONS*/
    ...
    /**
     * @return the txaCountryDescription
     */
    public TextArea getTxaCountryDescription() {
        return txaCountryDescription;
    }

    /**
     * @param txaCountryDescription the txaCountryDescription to set
     */
    public void setTxaCountryDescription(TextArea txaCountryDescription) {
        this.txaCountryDescription = txaCountryDescription;
    }
