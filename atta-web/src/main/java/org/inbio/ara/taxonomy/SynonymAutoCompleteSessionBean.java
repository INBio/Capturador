...
public class SynonymAutoCompleteSessionBean extends OptionsList implements AutoComplete {
    ...
    //private TaxonSessionBean tsb = null;


    //Constructor
    public SynonymAutoCompleteSessionBean() {
    ...
    public Long getIdSelected()
    {
       //return this.optionHash.get(text);
        return this.optionHash.get(text)[ID];
    }

    public Long getPosition(String name)
    {
       //return this.optionHash.get(text);
        return this.optionHash.get(name)[POSITION];
    }
    ...
    public boolean filter(String filter) {
        ...
        List<SynonymDTO> taxonDTOs =
                this.getTaxonomyFacade().getTaxonSynonymByName(filter, kingdomId, categoryId,0, MAX_RESULTS);
        int size = taxonDTOs.size();
        options = new Option[size];
        for(int i=0;i<size;i++){
            SynonymDTO aux = taxonDTOs.get(i);
            ...
            Long[] tmpArray = {aux.getTaxonKey(), new Long(i)};
            optionHash.put(aux.getDefaultName().trim(), tmpArray);
        }
