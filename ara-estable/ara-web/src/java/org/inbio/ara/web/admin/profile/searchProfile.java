 ...
 * searchProfile.java
 *
 * Created on 7 de febrero de 2008, 02:29 PM
 * Copyright herson
 */
 ...
 */
public class searchProfile extends AbstractPageBean {
    ...
    private TextField profileNameTF = new TextField();

    public TextField getProfileNameTF() {
        return profileNameTF;
    }

    public void setProfileNameTF(TextField tf) {
        this.profileNameTF = tf;
    }

    private Label name_label1 = new Label();

    public Label getName_label1() {
        return name_label1;
    }

    public void setName_label1(Label l) {
        this.name_label1 = l;
    }
    ...
    private TextField profileDescriptionTF = new TextField();

    public TextField getProfileDescriptionTF() {
        return profileDescriptionTF;
    }

    public void setProfileDescriptionTF(TextField tf) {
        this.profileDescriptionTF = tf;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("Module searchProfile initialization Failure", e);
    ...
    public String searchButton_action() {
        ...
        if(getProfileNameTF().getText() != null) {
            searchCriteria.put("lower(table.name) like ", 
            "'%" + getProfileNameTF().getText().toString().toLowerCase() + "%'");
        }
        if(getProfileDescriptionTF().getText() != null) {
            searchCriteria.put("lower(table.description) like ", 
            "'%" + getProfileDescriptionTF().getText().toString().toLowerCase() + "%'");
        }
        SearchManagerRemote smr = getadmin$profile$ProfileSessionBean().getSearchManager();
        List resultSet = smr.makeQuery(Profile.class, searchCriteria);
        if (resultSet != null) {
            getadmin$profile$ProfileSessionBean().setIsFiltered(true);
            getadmin$profile$ProfileSessionBean().getProfileDataProvider().clearObjectList();
            getadmin$profile$ProfileSessionBean().getProfileDataProvider().setList(resultSet);
            return "case1";
        }
        return null;
    }
    ...
    private boolean isValidInput() {
        ...
        if(getProfileNameTF().getText() != null) {
            String profile = getProfileNameTF().getText().toString();
            if (!profile.matches("[0-9a-zA-Záéíóú�?É�?ÓÚñÑ -]*")) {
                message += BundleHelper.getDefaultBundleValue("profile_name_error");
                isValid = false;
            }
        }
        if(getProfileDescriptionTF().getText() != null) {
            String description = getProfileDescriptionTF().getText().toString();
            if (!description.matches("[0-9a-zA-Záéíóú�?É�?ÓÚñÑ -]*")) {
