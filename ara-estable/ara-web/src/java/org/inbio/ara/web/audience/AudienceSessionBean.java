 ...
 */ 
/*
 * AudienceSessionBean.java
 *
 * Created on November 19, 2007, 10:47 PM
 * Copyright roaguilar
 */
 ...
 * this class.</p>
 */
public class AudienceSessionBean extends AbstractSessionBean {
    // <editor-fold defaultstate="collapsed" desc="Managed Component Definition">
    private int __placeholder;
    private AudienceDataProvider audienceDataProvider = new AudienceDataProvider();
    private String ejbMessage;
    private Audience audience;
    
    @EJB
    private AudienceRemote audienceBean;
    /**
    ...
    public void init() {
        ...
        try {
            ...
            log("AudienceSessionBean Initialization Failure", e);
    ...
    public AudienceDataProvider getAudienceDataProvider() {
        return audienceDataProvider;
    }

    public void setAudienceDataProvider(AudienceDataProvider audienceDataProvider) {
    ...
    public boolean create(Audience audience) {
        ...
        audienceBean = this.lookupAudienceBean();
        if (audienceBean.create(audience)) {
            getAudienceDataProvider().refreshDataList();
            created = true;
            this.getutil$MessageBean().addSuccessMessage("Registro creado con éxito");
        } else {
            this.setEjbMessage(audienceBean.getMessage());
    ...
    public boolean update(Audience audience) {
        ...
        audienceBean = this.lookupAudienceBean();
        if (audienceBean.update(audience)) {
    ...
    public boolean delete() {
        ...
        audienceBean = this.lookupAudienceBean();
        if (audienceBean.remove(this.getAudience().getId())) {
            getAudienceDataProvider().refreshDataList();
            deleted = true;
    ...
    public void setSelectedAudience(RowKey rowKey){
        if (rowKey != null) {
            setAudience((Audience)getAudienceDataProvider().getObject(rowKey));
        } else {
