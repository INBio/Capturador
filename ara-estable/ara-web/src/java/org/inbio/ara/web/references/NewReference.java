 ...
 */
public class NewReference extends AbstractPageBean {
	
	public static final String FIELD_IDENTIFIER_REQUIRED = BundleHelper.getDefaultBundleValue("field_identifier_required");
	public static final String FIELD_LANGUAGE_REQUIRED = BundleHelper.getDefaultBundleValue("field_language_required");
	public static final String FIELD_TITLE_REQUIRED = BundleHelper.getDefaultBundleValue("field_title_required");
	public static final String FIELD_TYPE_REQUIRED = BundleHelper.getDefaultBundleValue("field_type_required");
    ...
    MethodExpression actionMethod = expressionFactory.createMethodExpression(elContext,
            "#{references$NewReference.treeItemClickHandler}", String.class, new Class[] {});
    ...
    public void init() {
        ...
        try {
            ...
            log("Module newReference Initialization Failure", e);
    ...
    public void prerender() {
        this.txt_description.setMaxLength(500);
        this.txt_identifier.setMaxLength(500);
        this.setLanguageOptions(this.getspecies$SpeciesSessionBean().getLanguage());
        this.setReferenceTypeOptions(this.getreferences$ReferenceSessionBean().getReferenceType());
    }
    ...
    public String btn_save_action() {
        ...
        if (validateFields()) {
            ...
            reference.setLanguage(this.getspecies$SpeciesSessionBean().getLanguage((Long)this.dd_language.getSelected()));
            reference.setReferenceType(this.getreferences$ReferenceSessionBean().getReferenceType((Long)this.dd_referenceType.getSelected()));
            
            reference.setCreatedBy(userName);
            reference.setLastModificationBy(userName);
            
            if (this.getreferences$ReferenceSessionBean().create(reference)) {
    ...
    private boolean validateFields() {
        ...
        if (this.txt_title.getValue() == null) {
            this.getutil$MessageBean().addErrorMessage(FIELD_TITLE_REQUIRED);
            ok = false;
        }

        if (this.txt_identifier.getValue() == null) {
            this.getutil$MessageBean().addErrorMessage(FIELD_IDENTIFIER_REQUIRED);
            ok = false;
        }        
        if (this.dd_language.getValue().toString().equals("-1")) {
            this.getutil$MessageBean().addErrorMessage(FIELD_LANGUAGE_REQUIRED);
            ok = false;
        }        
        if (this.dd_referenceType.getValue().toString().equals("0")) {
            this.getutil$MessageBean().addErrorMessage(FIELD_TYPE_REQUIRED);
            ok = false;
        }
        return ok;
    }
