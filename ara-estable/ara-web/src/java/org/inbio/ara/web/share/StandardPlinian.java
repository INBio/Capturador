 ...
 * @version Created on 23/03/2009, 08:32:42 AM
 * @author esmata
 */
...
public class StandardPlinian extends AbstractPageBean {
    ...
    List<UIComponent> MycheckBoxList = getElementsPanel().getChildren(); // List of checkboxs components
    ...
    public void prerender() {
        //---------- Rendering the plinian core elements checkboxs ---------
        this.MycheckBoxList.clear();
        //Create and load the list with elements of Plinian standard
        LinkedList<StandardNode> ElementList = getshare$SessionBeanShare().findPliElements();
        ...
        LinkedList<QueryNode> sessionQueryList = getshare$SessionBeanShare().getQueryListPli();
    ...
    public String btnReload_action() {
    ...
    private QueryNode createQueryNode() {
        PliElement e = this.getshare$SessionBeanShare().getShareBR().getPliElementByIdB(new BigDecimal(1));
    ...
    private ArrayList<Option> getDwCElementsInOptionForm(){
        ArrayList<Option> pliElements = new ArrayList();
        List<PliElement> rawElements = this.getshare$SessionBeanShare().getShareBR().getPliElementsB();
        for (PliElement pliElement : rawElements) {
            pliElements.add (new Option(pliElement.getElementId(),
                    BundleHelper.getDefaultBundleValue(pliElement.
                        getElementKeyword())));
        }
        return pliElements;
    }
    ...
    private void drawDeleteLink(int row) {
        ...
        MethodExpression actionMethod = expressionFactory.createMethodExpression(elContext,
            "#{share$StandardPlinian.deleteClickHandler}", String.class, new Class[]{});

        MethodExpression listenerMethod = expressionFactory.createMethodExpression(elContext,
            "#{share$StandardPlinian.deleteClickHandler}",
    ...
    public void deleteClickHandler(ActionEvent ae) {
        ...
        getshare$SessionBeanShare().getQueryListPli().remove(index);
    }
    ...
    public String buPlinianShare_action() {
        // TODO: Process the action. Return value is a navigation
        // case name where null will return to the same page.
        return null;
    }
