 ...
 * EditReference.java
 *
 * Created on February 8, 2008, 10:12 AM
 * Copyright roaguilar
 */
 ...
 */
public class EditReference extends AbstractPageBean {
    ...
    MethodExpression actionMethod = expressionFactory.createMethodExpression(elContext,
            "#{references$EditReference.treeItemClickHandler}", String.class, new Class[] {});
    ...
    private Calendar cldr_publicationDate = new Calendar();

    public Calendar getCldr_publicationDate() {
        return cldr_publicationDate;
    }

    public void setCldr_publicationDate(Calendar c) {
        this.cldr_publicationDate = c;
    }
    ...
    private StaticText st_referenceId = new StaticText();

    public StaticText getSt_referenceId() {
        return st_referenceId;
    }

    public void setSt_referenceId(StaticText st) {
        this.st_referenceId = st;
    }
    ...
    private TextArea txt_data = new TextArea();

    public TextArea getTxt_data() {
        return txt_data;
    }

    public void setTxt_data(TextArea ta) {
        this.txt_data = ta;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("Module editReference Initialization Failure", e);
    ...
    public void prerender() {
        Reference reference = this.getreferences$ReferenceSessionBean().getReference();
        this.st_referenceId.setValue(reference.getId());
        this.txt_title.setValue(reference.getTitle());
        this.txt_description.setValue(reference.getDescription());
        this.txt_identifier.setValue(reference.getIdentifier());
        this.cldr_publicationDate.setSelectedDate(reference.getPublicationDate());
        this.initCategoriesTree();
        /*
        this.st_id.setText(audience.getId());
        this.txt_name.setText(audience.getName());
        this.txt_description.setText(audience.getDescription());
         */
    }
    ...
    public String btn_save_action() {
        ...
        if (validateFields()) {
            ...
            Reference reference = this.getreferences$ReferenceSessionBean().getReference();
            reference.setTitle((String)this.txt_title.getValue());
            reference.setIdentifier((String)this.txt_identifier.getValue());
            
            if (cldr_publicationDate.getSelectedDate() != null) {
                reference.setPublicationDate(cldr_publicationDate.getSelectedDate());
            }
            
            if (txt_description.getValue()!= null) {
                reference.setDescription((String)txt_description.getValue());
            }
            
            reference.setLanguage(this.getspecies$SpeciesSessionBean().getLanguage(this.getreferences$ReferenceSessionBean().getSelectedLanguage()));
            reference.setReferenceType(this.getreferences$ReferenceSessionBean().getReferenceType(this.getreferences$ReferenceSessionBean().getSelectedReferenceType()));
            
            reference.setLastModificationBy(userName);
            
            if (this.getreferences$ReferenceSessionBean().update(reference)) {
                return "case2";
            } else {
                return null;
            }
    ...
    private boolean validateFields() {
        ...
        if (this.txt_title.getValue() == null) {
            this.getutil$MessageBean().addErrorMessage("El título es obligatorio");
            ok = false;
        }

        if (this.txt_identifier.getValue() == null) {
            this.getutil$MessageBean().addErrorMessage("El identificador es obligatorio");
            ok = false;
        }        
        if (this.dd_language.getValue() == "-1") {
            this.getutil$MessageBean().addErrorMessage("El idioma es obligatorio");
            ok = false;
        }        
        if (this.dd_referenceType.getValue() == "-1") {
            this.getutil$MessageBean().addErrorMessage("El tipo es obligatorio");
            ok = false;
        }
        return ok;
    }
    ...
    private void initCategoriesTree(){
        // List of nodes
        List treeItems = this.getTree1().getChildren();
        if(treeItems.size() == 0){
            List<Object> elements = this.getreferences$ReferenceSessionBean().getReferenceElementList();
            treeItems = createTree(treeItems, elements);
        }
    }
    ...
    private List createTree(List treeItems, List toBeInsertItems){
        ...
        for(int i=0; i < toBeInsertItems.size(); i++){
            ...
            if (selectedNode != "") {
                tree1.setSelected(selectedNode);
            }
        }
        return treeItems;
    }
    ...
    public String treeItemClickHandler(){
        ...
        String nodeId = tree1.getSelected();
        TreeNode clickItem = (TreeNode)this.getForm1().findComponentById(nodeId);
        String[] treeNodeIdTokens = clickItem.getId().split("-");
        Long referenceElementId = Long.parseLong(treeNodeIdTokens[ID_POSITION]);
        
        this.updateReferecenElementValue();
        this.getreferences$ReferenceSessionBean().setSelectedReferenceElement(referenceElementId);
        
        this.txt_data.setValue(this.getreferences$ReferenceSessionBean().getReferenceElementValue(referenceElementId));
        this.getreferences$ReferenceSessionBean().setSelectedReferenceElement(referenceElementId);
        /*
        cleanWidgetsArea();
        ...
        try{
            ...
            TaxonDescriptionCategory taxonDescriptionCategory = this.getAraSessionBean().getTaxonDescriptionCategory(Long.parseLong(treeNodeIdTokens[ID_POSITION]));
            if (taxonDescriptionCategory != null) {
                if (taxonDescriptionCategory.getRepeatable().equals(0L)) {
                    // La categoría no usa repeticiones, se dibuja solamente el formulario dinámico.
                    selectLeaf(categoryId);
                    this.getAraSessionBean().setCurrentTaxonDescriptionCategoryId(categoryId);
                    //this.getAraSessionBean().setIsDynamicFormPanelActive(true);
                } else {
                    // La categoría usa repeticiones, se dibuja solamente la tabla dinámico.
                    ...
                    //btnAddTemp.setOnClick("if (window.showModalDialog) {\n    window.showModalDialog('newTaxonDescriptionRecord.jsp','name','dialogWidth:510px;dialogHeight:500px');\n} else {\n    window.open('newTaxonDescriptionRecord.jsp','name','height=510px,width=500px,toolbar=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no ,modal=yes');\n}\n");
    ...
    public boolean updateReferecenElementValue() {
        String data = (String)this.txt_data.getValue();
        if (data == null) {
            return true;
        } else {
            return this.getreferences$ReferenceSessionBean().updateReferenceElementValue(data);
        }
    }
