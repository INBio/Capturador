 ...
 */ 
/*
 * editNomenclaturalGroup.java
 *
 * Created on June 29, 2008, 10:46 PM
 * Copyright roaguilar
 */
...
public class editNomenclaturalGroup extends AbstractPageBean {
	...
	public static final String RECORD_MODIFICATION_SUCCESS = BundleHelper.getDefaultBundleValue("modification_success");
    ...
    private Tab tab_taxon1 = new Tab();

    public Tab getTab_taxon1() {
        return tab_taxon1;
    }

    public void setTab_taxon1(Tab t) {
        this.tab_taxon1 = t;
    }
    ...
    private Tab tab_region1 = new Tab();

    public Tab getTab_region1() {
        return tab_region1;
    }

    public void setTab_region1(Tab t) {
        this.tab_region1 = t;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("Module editNomenclaturalGroup Initialization Failure", e);
    ...
    public void prerender() {
        ...
        if (!this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().isEditMode()) {
            this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().setEditMode(true);
            this.st_id.setValue(this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getId());
            this.txt_name.setValue(this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getName());
            this.txt_description.setValue(this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getDescription());
            this.txt_temporality.setValue(this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getTemporality());
            this.txt_notes.setValue(this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getNotes());
			this.dd_commonName.setSelected(this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getCommonName());
            /*
            if (this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup().getCommonName().equals("y")) {
                //this.chck_commonName.setSelected(1);
                this.chck_commonName.setValue(true);
            } else {
                this.chck_commonName.setSelected(0);
            }
             */
        }
    }
    ...
    /**
     * Con solo que se vaya al server se actualiza la jugada
     */
    public void btn_update(){

		Long[] selectedTaxon = {};

		this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().setSelectedTaxon(selectedTaxon);
    ...
    public String btn_save_action() {
        if (validate()) {
            NomenclaturalGroup obj = this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getNomenclaturalGroup();
           ...
           obj.setCommonName(this.dd_commonName.getValue().toString());
            /*
            ...
            */
            ...
            if (this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().update(obj)) {
                this.getutil$MessageBean().addSuccessMessage(RECORD_MODIFICATION_SUCCESS);
                return "SaveNomenclaturalGroup";
            } else {
                return null;
            }
        } else {
    ...
    public String btn_cancel_action() {
        return "CancelEditNomenclaturalGroup";
    }
    
    public boolean validate() {
        if (this.txt_name.getValue() == null) {
            this.getutil$MessageBean().addErrorMessage(EMPTY_NAME);
            return false;
        }
		/*
        if (this.getnomenclaturalgroup$NomenclaturalGroupSessionBean().getSelectedCertificator() == -1l) {
            this.getutil$MessageBean().addErrorMessage(EMPTY_CERTIFIER);
            return false;
        }
		 */
        return true;
