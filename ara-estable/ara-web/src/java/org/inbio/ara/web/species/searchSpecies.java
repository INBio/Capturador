 ...
 * searchSpecies.java
 *
 * Created on 8 de febrero de 2008, 08:39 AM
 * Copyright herson
 */
 ...
 */
public class searchSpecies extends AbstractPageBean {
    ...
    private Label default_name_label = new Label();

    public Label getDefault_name_label() {
        return default_name_label;
    }

    public void setDefault_name_label(Label l) {
        this.default_name_label = l;
    }

    private TextField scientificNameTF = new TextField();

    public TextField getScientificNameTF() {
        return scientificNameTF;
    }

    public void setScientificNameTF(TextField tf) {
        this.scientificNameTF = tf;
    }
    ...
    private Label kingdom_label = new Label();

    public Label getKingdom_label() {
        return kingdom_label;
    }

    public void setKingdom_label(Label l) {
        this.kingdom_label = l;
    }

    private TextField kingdomTF = new TextField();

    public TextField getKingdomTF() {
        return kingdomTF;
    }

    public void setKingdomTF(TextField tf) {
        this.kingdomTF = tf;
    }

    private Label family_label = new Label();

    public Label getFamily_label() {
        return family_label;
    }

    public void setFamily_label(Label l) {
        this.family_label = l;
    }

    private TextField familyTF = new TextField();

    public TextField getFamilyTF() {
        return familyTF;
    }

    public void setFamilyTF(TextField tf) {
        this.familyTF = tf;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log(java.util.ResourceBundle.getBundle("org/inbio/ara/web/species/resources").getString("speciesSearch_Initialization_Failure"), e);
    ...
    /**
     * Performs validation over user input
    ...
    private boolean isValidInput(){
        ...
        if (getScientificNameTF().getText() != null) {
            String scientificName = getScientificNameTF().getText().toString();
            if (!scientificName.matches("[a-zA-Z0-9 ]*")) {
                message += ResourceBundle.getBundle("org/inbio/ara/web/species/resources").getString("scientificNameError");
                isValid = false;
            }
        }
        if(getKingdomTF().getText() != null) {
            String kingdom = getKingdomTF().getText().toString();
            if (!kingdom.matches("[a-zA-Z0-9 ]*")) {
                message += ResourceBundle.getBundle("org/inbio/ara/web/species/resources").getString("kingdomError");
                isValid = false;
            }
        }
        if(getFamilyTF().getText() != null) {
            String family = getFamilyTF().getText().toString();
            if (!family.matches("[a-zA-Z0-9 ]*")) {                
                message += ResourceBundle.getBundle("org/inbio/ara/web/species/resources").getString("familyError");;
                isValid = false;
            }
        }
        if (!isValid) {
            ...
            searchAlert.setSummary(ResourceBundle.getBundle("org/inbio/ara/web/species/resources").getString("validation_error"));
    ...
    /**
     * This method creates the main section of the query based on the user input
     * @return String defined in navigation.xml
     */
    public String searchButton_action() {
        ...
        if(getScientificNameTF().getText() != null) {
            ...
            "'%" + getScientificNameTF().getText().toString().toLowerCase() + "%'");
        }
        if(getKingdomTF().getText() != null) {
            /*searchCriteria.put("table.kingdomTaxon in (select table.id from table where table.defaultName like ", 
                  "'%" + getKingdomTF().getText().toString() + "%')");*/
            searchCriteria.put("lower(table.taxon.kingdomTaxon.defaultName) like ", 
            "'%" + getKingdomTF().getText().toString().toLowerCase() + "%')");
        }
        if(getFamilyTF().getText() != null) {
            /*searchCriteria.put("table.familyTaxon in (select table.id from table where table.defaultName like ", 
                  "'%" + getFamilyTF().getText().toString() + "%')");*/
            searchCriteria.put("lower(table.taxon.familyTaxon.defaultName) like ", 
            "'%" + getFamilyTF().getText().toString().toLowerCase() + "%')");
        }
        
        SearchManagerRemote smr = this.getAraRequestBean().getSearchManager();
		List resultSet = smr.makeQuery(TaxonDescription.class, searchCriteria);
        //List resultSet = null;
        if (resultSet != null) {
            getspecies$SpeciesSessionBean().setFiltered(true);
            //getAraSessionBean().getTaxonDataProvider().clearObjectList();
            //getAraSessionBean().getTaxonDataProvider().setList(resultSet);
            getspecies$SpeciesSessionBean().getTaxonDescriptionDataProvider().clearObjectList();
            
            
            
            System.out.println("////*/*/*/*/*"+resultSet.size());
            List smallSpeciesList = new ArrayList();
            for (Object elem : resultSet) {
                TaxonDescription taxonDescription = (TaxonDescription) elem;
            ...
            getspecies$SpeciesSessionBean().getTaxonDescriptionDataProvider().setList(smallSpeciesList);
            return "case1";
        }
        return null;              
    }
