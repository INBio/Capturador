 ...
 * NewStage.java
 *
 * Created on 26 de marzo de 2008, 04:49 PM
 * Copyright herson
 */
 ...
 */
public class NewStage extends AbstractPageBean {
	...
	public static final String STAGE_CREATE_SUCCESS = BundleHelper.getDefaultBundleValue("stage_create_success");
    ...
    public void init() {
        ...
        try {
            ...
            log("NewStage Initialization Failure", e);
    ...
    public String btn_save_action() {
        if(!isValidInput())
            return null;
        TaxonDescriptionStage TDStage = new TaxonDescriptionStage();

        TDStage.setName(getName_field().getText().toString());
        TDStage.setDescription(getDescription_area().getText().toString());
        try {
            TDStage.setCreatedBy(getSessionManager().getUser().getUserName());
        ...
        if (getAraRequestBean().getTaxonDescriptionStageBean().create(TDStage)){
            System.out.println("todo bien");
            this.getutil$MessageBean().addSuccessMessage(STAGE_CREATE_SUCCESS);
            getAraApplicationBean().getStagesDP().refreshDataList();
        } else {
            this.getutil$MessageBean().addErrorMessage(STAGE_CREATE_SUCCESS);
