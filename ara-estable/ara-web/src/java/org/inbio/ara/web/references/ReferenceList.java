 ...
 */
public class ReferenceList extends AbstractPageBean {

	
	public static final String RECORD_DELETION_SUCCESS = BundleHelper.getDefaultBundleValue("record_deletion_success");
	...
	public void init() {
		...
		if (this.getSessionManager().getUser() == null) {
			try {
				externalContext.redirect(externalContext.getRequestContextPath() + "/faces/login.jsp");
		...
		try {
			...
			log("ReferenceList Initialization Failure", e);
	...
	public void prerender() {
		getreferences$ReferenceSessionBean().initDataProvider();
	}
	...
	public String btn_remove_action() {
		if (this.getSessionManager().canDelete() != null) {
			RowKey rowKey = this.tableRowGroup1.getRowKey();
			this.getreferences$ReferenceSessionBean().setSelectedReference(rowKey);
			if (getreferences$ReferenceSessionBean().delete()) {
				this.getutil$MessageBean().addSuccessMessage(RECORD_DELETION_SUCCESS);
			} else {
				this.getutil$MessageBean().addErrorMessage(getreferences$ReferenceSessionBean().getEjbMessage());
			}
		} else {
	...
	public String btn_showAll_action() {
		...
		this.getreferences$ReferenceSessionBean().referenceDataProvider.refreshDataList();
	...
	protected SpeciesSessionBean species$getSpeciesSessionBean() {
		return (SpeciesSessionBean) getBean("species$SpeciesSessionBean");
	}
