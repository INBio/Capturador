 ...
 */
/*
 * listInstitution.java
 *
 * Created on November 11, 2007, 1:18 PM
 * Copyright roaguilar
 */
...
public class listInstitution extends AbstractPageBean {
	...
	private static final String INIT_FAILED = BundleHelper.getDefaultBundleValue("init_failed");
	...
	private void _init() throws Exception {
		institutionTable.setStyle("left: 0px; top: 216px; position: absolute; width: 100%");
	}
	...
	private Table institutionTable = new Table();

	public Table getInstitutionTable() {
		return institutionTable;
	}

	public void setInstitutionTable(Table t) {
		this.institutionTable = t;
	}
	...
	public void prerender() {
		this.getadmin$institution$InstitutionSessionBean().initDataProvider();
	}
	...
	public String btn_showAll_action() {
		...
		this.getadmin$institution$InstitutionSessionBean().getInstitutionDataProvider().refreshDataList();
	...
	public String btn_edit_action() {
		if (this.getSessionManager().canModify() != null) {
			...
			this.getadmin$institution$InstitutionSessionBean().setSelectedInstitution(rowKey);
	...
	public String btn_cancel_action() {
		if (this.getSessionManager().canDelete() != null) {
			...
			this.getadmin$institution$InstitutionSessionBean().setSelectedInstitution(rowKey);
			if (getadmin$institution$InstitutionSessionBean().delete()) {
