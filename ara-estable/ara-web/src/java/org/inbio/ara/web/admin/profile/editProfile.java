 ...
 */
public class editProfile extends AbstractPageBean {
	
	private static final String EMPTY_PROFILE_NAME = BundleHelper.getDefaultBundleValue("empty_profile_name");
	private static final String ERROR = BundleHelper.getDefaultBundleValue("error");
	private static final String SAVE_FAILED = BundleHelper.getDefaultBundleValue("save_failed");
	private static final String VALIDATION_ERROR = BundleHelper.getDefaultBundleValue("validation_error");
	private static final String WARNING = BundleHelper.getDefaultBundleValue("warning");
    ...
    public void setLayoutPanel(PanelLayout pl) {
        this.layoutPanel = pl;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("Module editProfile Initialization Failure", e);
    ...
    public void prerender() {
        Profile profile = getadmin$profile$ProfileSessionBean().getProfile();
        
        this.st_id.setText(profile.getId());
        this.txt_name.setText(profile.getName());
        this.txt_description.setText(profile.getDescription());
    }
    ...
    private boolean validateFields() {
        ...
        if (this.txt_name.getValue() == null) {
            profileAlert.setType(WARNING);          
            profileAlert.setSummary(VALIDATION_ERROR);
            profileAlert.setDetail(EMPTY_PROFILE_NAME);
            ok = false;
        }
        return ok;
    }
    ...
    public String btn_save_action() {
        ...
        if (this.validateFields()) {
            Profile profile = this.getadmin$profile$ProfileSessionBean().getProfile();

            profile.setName((String)this.txt_name.getValue());
            profile.setDescription((String)this.txt_description.getValue());
            if (this.getadmin$profile$ProfileSessionBean().update(profile)) {
                return "doSaveProfile";
            } else {
                profileAlert.setType(ERROR);          
                profileAlert.setSummary(SAVE_FAILED);
                profileAlert.setDetail(getadmin$profile$ProfileSessionBean().getEjbMessage());
                return null;
            }
        } else {
            return null;
