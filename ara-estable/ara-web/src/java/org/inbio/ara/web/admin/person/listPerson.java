 ...
 * listPerson.java
 *
 * Created on September 30, 2007, 1:49 PM
 * Copyright roaguilar
 */
...
public class listPerson extends AbstractPageBean {
	...
	//private static final String DELETETION_SUCCESS = BundleHelper.getDefaultBundleValue("deletion_success");
    private static final String DELETETION_SUCCESS_KEY = "deletion_success";
    ...
    private Table personTable = new Table();

    public Table getPersonTable() {
        return personTable;
    }

    public void setPersonTable(Table t) {
        this.personTable = t;
    }
    ...
    private Alert personAlert = new Alert();

    public Alert getPersonAlert() {
        return personAlert;
    }

    public void setPersonAlert(Alert a) {
        this.personAlert = a;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("Modulo listPerson initialization Failure", e);
    ...
    public void preprocess() {
        //this.getadmin$person$PersonSessionBean().getPersonDataProvider().clearObjectList();
    ...
    public void prerender() {
        this.getadmin$person$PersonSessionBean().initDataProvider();
        this.getPersonSessionBean().setEditMode(false);
    }
	...
	private Button btn_showAll = new Button();

	public Button getBtn_showAll() {
		return btn_showAll;
	}

	public void setBtn_showAll(Button b) {
		this.btn_showAll = b;
	}
    ...
    public String button1_action() {
        if (this.getSessionManager().canModify()!=null) {
            ...
            getadmin$person$PersonSessionBean().setSelectedPerson(rowKey);
            getadmin$person$PersonSessionBean().populateLists();
    ...
    public String button2_action() {
        if (this.getSessionManager().canDelete() != null) {
            RowKey rowKey = this.tableRowGroup1.getRowKey();
            getadmin$person$PersonSessionBean().setSelectedPerson(rowKey);
            if (getadmin$person$PersonSessionBean().delete()) {
                MessageBean.setErrorMessageFromBundle(DELETETION_SUCCESS_KEY);
                //this.getutil$MessageBean().addSuccessMessage(DELETETION_SUCCESS);
            } else {
                MessageBean.addErrorMessage(getadmin$person$PersonSessionBean().getEjbMessage());
            }
            return null;
        } else {
            //this.getutil$MessageBean().addCantDeleteMessage();
            MessageBean.setErrorMessageFromBundle("cant_delete_registries");
            return null;
        }
    }
    ...
    public String btn_new_action() {
        if (this.getSessionManager().canAdd() != null) {
            this.getadmin$person$PersonSessionBean().setSelectedProfiles(new Long[]{});
            this.getadmin$person$PersonSessionBean().setSelectedInstitutions(new Long[]{});
    ...
    private void setMessage(String type, String summary, String detail) {
        this.personAlert.setType(type);
        this.personAlert.setSummary(summary);
        this.personAlert.setDetail(detail);
    }
