 ...
 * listProfile.java
 *
 * Created on November 12, 2007, 1:07 AM
 * Copyright roaguilar
 */
...
public class listProfile extends AbstractPageBean {
	...
	private static final String DELETION_SUCCESS = BundleHelper.getDefaultBundleValue("deletion_success");
	...
	private void _init() throws Exception {
		profileTable.setStyle("left: 0px; top: 192px; position: absolute; width: 100%");
	}
	...
	private Alert profileAlert = new Alert();

	public Alert getProfileAlert() {
		return profileAlert;
	}

	public void setProfileAlert(Alert a) {
		this.profileAlert = a;
	}
	...
	public void setProfileTable(Table t) {
		this.profileTable = t;
	}
	...
	private Button btn_newProfile = new Button();

	public Button getBtn_newProfile() {
		return btn_newProfile;
	}

	public void setBtn_newProfile(Button b) {
		this.btn_newProfile = b;
	}
	private Button btn_searchProfile = new Button();

	public Button getBtn_searchProfile() {
		return btn_searchProfile;
	}

	public void setBtn_searchProfile(Button b) {
		this.btn_searchProfile = b;
	}
	...
	public void init() {
		...
		try {
			...
			log("Module listProfile initialization Failure", e);
	...
	public void prerender() {
		this.getadmin$profile$ProfileSessionBean().initDataProvider();
	}
	...
	public String btn_showAll_action() {
		...
		this.getadmin$profile$ProfileSessionBean().profileDataProvider.refreshDataList();
	...
	public String btn_newProfile_action() {
	...
	public String btn_searchProfile_action() {
		return "search";
	}
	...
	public String btn_edit_action() {
		if (this.getSessionManager().canModify() != null) {
			...
			getadmin$profile$ProfileSessionBean().setSelectedProfile(rowKey);
	...
	public String btn_remove_action() {
		if (this.getSessionManager().canDelete() != null) {
			RowKey rowKey = this.tableRowGroup1.getRowKey();
			getadmin$profile$ProfileSessionBean().setSelectedProfile(rowKey);
			if (getadmin$profile$ProfileSessionBean().delete()) {
				this.getutil$MessageBean().addSuccessMessage(DELETION_SUCCESS);
			} else {
				this.getutil$MessageBean().addErrorMessage(getadmin$profile$ProfileSessionBean().getEjbMessage());
			}
		} else {
	...
	private void setMessage(String type, String summary, String detail) {
		this.profileAlert.setType(type);
		this.profileAlert.setSummary(summary);
		this.profileAlert.setDetail(detail);
	}
