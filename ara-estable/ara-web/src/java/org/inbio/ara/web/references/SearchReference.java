...
/*
 * SearchReference.java
 *
 * Created on 15 de abril de 2008, 09:56 AM
 * Copyright herson
 */
 ...
 */
public class SearchReference extends AbstractPageBean {

	public static final String ALLOWED_CHARS = "[a-zA-Záéíóú�?É�?ÓÚñÑ0-9 -]*";
	public static final String ERROR = BundleHelper.getDefaultBundleValue("error");
	public static final String FIELD_DESCRIPTION_ERROR = BundleHelper.getDefaultBundleValue("field_description_error");
	public static final String FIELD_IDENTIFIER_ERROR = BundleHelper.getDefaultBundleValue("field_identifier_error");
	public static final String FIELD_LANGUAGE_ERROR = BundleHelper.getDefaultBundleValue("field_language_error");
	public static final String FIELD_TITLE_ERROR = BundleHelper.getDefaultBundleValue("field_title_error");
	public static final String TYPE_LANGUAGE_ERROR = BundleHelper.getDefaultBundleValue("type_language_error");
	public static final String VALIDATION_ERROR = BundleHelper.getDefaultBundleValue("validation_error");
	...
	public void setTitleTF(TextField tf) {
		this.titleTF = tf;
	}
	...
	public void setDescriptionTF(TextField tf) {
		this.descriptionTF = tf;
	}
	...
	private TextField identifierTF = new TextField();

	public TextField getIdentifierTF() {
		return identifierTF;
	}

	public void setIdentifierTF(TextField tf) {
		this.identifierTF = tf;
	}
	...
	private DropDown dd_referenceType = new DropDown();

	public DropDown getDd_referenceType() {
		return dd_referenceType;
	}

	public void setDd_referenceType(DropDown dd) {
		this.dd_referenceType = dd;
	}
	private SingleSelectOptionsList dd_referenceTypeDefaultOptions = new SingleSelectOptionsList();

	public SingleSelectOptionsList getDd_referenceTypeDefaultOptions() {
		return dd_referenceTypeDefaultOptions;
	}

	public void setDd_referenceTypeDefaultOptions(SingleSelectOptionsList ssol) {
		this.dd_referenceTypeDefaultOptions = ssol;
	}
	private DropDown dd_language = new DropDown();

	public DropDown getDd_language() {
		return dd_language;
	}

	public void setDd_language(DropDown dd) {
		this.dd_language = dd;
	}
	private SingleSelectOptionsList dd_languageDefaultOptions = new SingleSelectOptionsList();

	public SingleSelectOptionsList getDd_languageDefaultOptions() {
		return dd_languageDefaultOptions;
	}

	public void setDd_languageDefaultOptions(SingleSelectOptionsList ssol) {
		this.dd_languageDefaultOptions = ssol;
	}
	...
	public void init() {
		...
		try {
			...
			log("Module searchReference Initialization Failure", e);
	...
	public void prerender() {
		this.getDd_languageDefaultOptions().setOptions(this.getspecies$SpeciesSessionBean().getLanguage());
		this.getDd_referenceTypeDefaultOptions().setOptions(getreferences$ReferenceSessionBean().getReferenceType());
	}
	...
	public String searchButton_action() {
		...
		if (getTitleTF().getText() != null) {
			searchCriteria.put("lower(table.title) like ",
					"'%" + getTitleTF().getText().toString().toLowerCase() + "%'");
		}
		if (getDescriptionTF().getText() != null) {
			searchCriteria.put("lower(table.description) like ",
					"'%" + getDescriptionTF().getText().toString().toLowerCase() + "%'");
		}
		if (getIdentifierTF().getText() != null) {
			searchCriteria.put("lower(table.identifier) like ",
					"'%" + getIdentifierTF().getText().toString().toLowerCase() + "%'");
		}
		if (this.getDd_language().getSelected() != null) {
			//las opciones tienen id 1 o superior
			if (getreferences$ReferenceSessionBean().getSelectedLanguage() > 0) {
				searchCriteria.put("table.language.languageId = ",
						getreferences$ReferenceSessionBean().getSelectedLanguage().toString());
			}
		}
		if (this.getDd_referenceType().getSelected() != null) {
			//las opciones tienen id 1 o superior
			if (getreferences$ReferenceSessionBean().getSelectedReferenceType() > 0) {
				searchCriteria.put("table.referenceType.id = ",
						getreferences$ReferenceSessionBean().getSelectedReferenceType().toString());
			}
		}

		SearchManagerRemote smr = getreferences$ReferenceSessionBean().getSearchManager();
		List resultSet = smr.makeQuery(Reference.class, searchCriteria);
		if (resultSet != null) {
			getreferences$ReferenceSessionBean().setFiltered(true);
			getreferences$ReferenceSessionBean().getReferenceDataProvider().clearObjectList();
			getreferences$ReferenceSessionBean().getReferenceDataProvider().setList(resultSet);
			return "list";
		}
		return null;
	}
	...
	private boolean isValidInput() {
		...
		if (getTitleTF().getText() != null) {
			String title = getTitleTF().getText().toString();

			if (!title.matches(ALLOWED_CHARS)) {
				message += FIELD_TITLE_ERROR;
				isValid = false;
			}

		}
		if (getDescriptionTF().getText() != null) {
			String description = getDescriptionTF().getText().toString();

			if (!description.matches(ALLOWED_CHARS)) {
				message += FIELD_DESCRIPTION_ERROR;
				isValid = false;
			}

		}
		if (getIdentifierTF().getText() != null) {
			String identifier = getIdentifierTF().getText().toString();

			if (!identifier.matches(ALLOWED_CHARS)) {
				message += FIELD_IDENTIFIER_ERROR;
				isValid = false;
			}
		...
		if (this.getDd_language().getSelected() != null) {
			try {
				Long language = getreferences$ReferenceSessionBean().getSelectedLanguage();
			} catch (Exception e) {
				// message += 	FIELD_LANGUAGE_ERROR;
				isValid = false;
			}
		}
		if (this.getDd_referenceType() != null) {
			try {
				Long type = getreferences$ReferenceSessionBean().getSelectedReferenceType();
			} catch (Exception e) {
				// message += 	TYPE_LANGUAGE_ERROR;
				isValid = false;
			}
		}
		...
		if (!isValid) {
			...
			searchAlert.setSummary(VALIDATION_ERROR);
