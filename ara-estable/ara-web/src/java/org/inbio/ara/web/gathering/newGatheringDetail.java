 ...
 */ 
/*
 * newGatheringDetail.java
 *
 * Created on June 1, 2008, 4:07 PM
 * Copyright roaguilar
 */
...
public class newGatheringDetail extends AbstractPageBean {
	...
	public static final String EMPTY_COLLECTOR_NUMBER = BundleHelper.getDefaultBundleValue("empty_collector_number");
    ...
    private StaticText txt_collectionName = new StaticText();

    public StaticText getTxt_collectionName() {
        return txt_collectionName;
    }

    public void setTxt_collectionName(StaticText st) {
        this.txt_collectionName = st;
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("Module newGatheringDetail initialization Failure", e);
    ...
    public void prerender() {
        if (this.getSessionManager().morphologicalDescriptionModuleName().equals("specimen")) {
        ...
        this.txt_collectionName.setValue(this.getSessionManager().getCollection().getName());
    }
    ...
    public String btn_cancel_action() {
        ...
        return "cancelNewGatheringDetail";
    }
    ...
    public String btn_save_action() {
        if (this.getSessionManager().morphologicalDescriptionModuleName().equals("gatheringDetail")) {
         ...
         if (validate()) {
             GatheringObservationDetail detail = new GatheringObservationDetail();
             detail.setCollection(this.getSessionManager().getCollection());
             detail.setGatheringObservation(this.getgathering$GatheringSessionBeanV2().getGatheringObservation());
             detail.setPerson(this.getutil$SelectionListBean().getPerson(this.getgathering$GatheringDetailSessionBean().getSelectedCollector()));
             detail.setCreatedBy(this.getSessionManager().getUser().getUserName());
             detail.setLastModificationBy(this.getSessionManager().getUser().getUserName());
             detail.setGatheringObservationDetailNumber((String)this.txt_gatheringNumber.getValue());
             if (this.getgathering$GatheringDetailSessionBean().createGatheringObservationDetail(detail)) {
                this.getgathering$GatheringSessionBeanV2().getGatherinDetailDataProvider().refreshDataList();
                return "saveNewGatheringDetail";
             } else {
                 return null;
             }
         } else {
    ...
    private boolean validate() {
        ...
        if (this.txt_gatheringNumber.getValue() == null) {
            this.getutil$MessageBean().addErrorMessage(EMPTY_COLLECTOR_NUMBER);
            return false;
        }
        return true;
    }
